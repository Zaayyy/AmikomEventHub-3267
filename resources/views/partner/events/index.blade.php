...
@section('title', 'Kelola Event - Partner')
@section('page_title', 'Kelola Event')
@section('page_subtitle', 'Buat dan atur event Anda sendiri di sini.')

@section('content')

<div class="mb-4 text-right">
    <a href="{{ route('partner.events.create') }}" class="inline-block px-6 py-3 bg-indigo-600 text-white rounded-2xl font-bold shadow-lg shadow-indigo-100 hover:bg-indigo-700 active:scale-95 transition">
        + Tambah Event Baru
    </a>
</div>
        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y border-t">
                ...
                <tr class="hover:bg-slate-50/50 transition">
                    ...
                    <td class="px-8 py-6">
                        <img src="{{ ($event->poster_path && \Storage::disk('public')->exists($event->poster_path)) ? asset('storage/' . $event->poster_path) : 'https://placehold.co/400x500?text=No+Poster' }}"
                             alt="{{ $event->title }}"
                    ...
                    <td class="px-8 py-6">
                        <div class="flex gap-2">
                            <a href="{{ route('partner.events.edit', $event->id) }}" class="p-2.5 bg-indigo-50 text-indigo-600 rounded-xl hover:bg-indigo-600 hover:text-white transition">
                            ...
                            <form action="{{ route('partner.events.destroy', $event->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus acara ini?');">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                <tr>
                    <td colspan="5" class="px-8 py-10 text-center text-slate-500">Anda belum menambahkan event apapun.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
